...
<\/script>
</body></html>`}]},{id:"cpp",icon:"🔷",name:"C++",nameAr:"سي++",desc:"University C++ starter",descAr:"بداية C++ للجامعة",files:[{name:"main.cpp",lang:"cpp",code:`#include <iostream>
#include <vector>
using namespace std;

// Reads numbers, then prints their sum and the largest one.
int largest(const vector<int>& v) {
    int best = v[0];
    for (int x : v) if (x > best) best = x;
    return best;
}
...
int main() {
    ...
    return 0;
}`}]},{id:"java",icon:"☕",name:"Java",nameAr:"جافا",desc:"Classes and input",descAr:"كلاسات وإدخال",files:[{name:"Main.java",lang:"java",code:`import java.util.Scanner;

public class Main {

    // A small class, the way OOP courses introduce it
    static class Student {
        ...
        boolean passed() { return grade >= 60; }
    }

    public static void main(String[] args) {
        Scanner in = new Scanner(System.in);
        System.out.print("Student name: ");
        String name = in.hasNext() ? in.next() : "Sara";
        System.out.print("Grade: ");
        double grade = in.hasNextDouble() ? in.nextDouble() : 85;

        Student s = new Student(name, grade);
        System.out.println(s.name + " -> " + (s.passed() ? "Passed" : "Failed"));
    }
}`}]},{id:"c",icon:"🔵",name:"C",nameAr:"سي",desc:"Arrays and loops",descAr:"مصفوفات وحلقات",files:[{name:"main.c",lang:"c",code:`#include <stdio.h>

/* Classic intro exercise: read n numbers, print the average. */
int main(void) {
    int n;
    ...
    printf("Average: %.2f\n", sum / n);
    return 0;
}`}]},{id:"csharp",icon:"🟣",name:"C#",nameAr:"سي شارب",desc:"Methods and loops",descAr:"دوال وحلقات",files:[{name:"Program.cs",lang:"csharp",code:`using System;

class Program {
    // A method plus a loop: the shape most C# courses start with
    ...
    static void Main() {
        ...
        int n = int.TryParse(line, out int v) ? v : 5;

        Console.WriteLine($"{n}! = {Factorial(n)}");
    }
}`}]},{id:"sql",icon:"🗄️",name:"SQL",nameAr:"قواعد بيانات",desc:"Create, insert, select",descAr:"إنشاء وإدخال واستعلام",files:[{name:"main.sql",lang:"sql",code:`-- Databases course starter: create, fill, then query.
...
-- Everyone who passed, best first
SELECT name, grade
FROM students
WHERE grade >= 60
ORDER BY grade DESC;`}]},{id:"python3",icon:"🐍",name:"Python 3",nameAr:"بايثون 3",desc:"Functions and lists",descAr:"دوال وقوائم",files:[{name:"main.py",lang:"python",code:`# Intro exercise: read numbers and describe them.

def stats(values):
    return min(values), max(values), sum(values) / len(values)

numbers = [12, 7, 45, 3, 28]

low, high, avg = stats(numbers)
