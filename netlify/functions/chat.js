/* ========================================================================
   Vixo AI 🤖 — the smart assistant (Netlify Serverless Function)
   ------------------------------------------------------------------------
...
const SYSTEM_PROMPT =
  'You are Vixo AI, the official smart assistant of the Vixo platform — a bilingual ' +
  ...
  'if asked, say you are "Vixo AI, the Vixo assistant".';
...
exports.handler = async function (event) {
  ...
  /* Abuse guard: only serve requests that come from our own site, so other
     sites can't burn the quota. The list has to track the deployed domain —
     it was iqtaani-only, which 403'd every request once the site moved to
     vixo. Lenient — a missing Origin/Referer is allowed
  ...
  if (_origin && !/vixo|iqtaani|localhost|127\.0\.0\.1/i.test(_origin)) {
